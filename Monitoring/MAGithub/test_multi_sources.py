...
from Monitoring.SourceIdentification import Camerini
...
def process(steps, graph, k, interval, writer=None, file_version=False):
    ...
    camerini = Camerini.Camerini(graph, attr='act_prob')
    ...
    writer.writerow(list(random_sources) + [len(infected_nodes), edges] + sources + [accuracy_algo, time_algo] + list(
        imeter_solutions) + [accuracy_IMeterSort, time_IMeterSort])
