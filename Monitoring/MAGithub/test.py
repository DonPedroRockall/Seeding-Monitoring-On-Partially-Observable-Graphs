...
from Monitoring.SourceIdentification.Camerini import *
...
def run_exp(steps, pathname, interval, random_node=None, writer=None):
    infected_nodes = []
    ...
    writer.writerow(
        [random_node, len(infected_nodes), len(infected_subgraph.edges()), source, dist_algo, time_algo, max_mem_usage,
         source_IMeterSort, dist_IMeterSort, time_IMeterSort])
    return True
...
if os.stat(pathname_output).st_size == 0:
    heading = ['Source', 'Num_Infected_Nodes', 'Num_Edges', 'Algo', 'Dist_Algo', 'Time_Algo', 'Mem_Usage', 'IMeterSort',
               'Dist_IMeterSort', 'Time_IMeterSort']
...
for i in range(0, len(intervals), 2):
    ...
    while j < n_exp:
        ...
        res = run_exp(steps, pathname_graph, intervals[i:i + 2], random_node=str(node), writer=writer)
        if res:
            j += 1
