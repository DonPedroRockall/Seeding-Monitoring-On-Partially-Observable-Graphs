...
from Monitoring.SourceIdentification.EdmondsMemoryOpt import maximum_spanning_arborescence_manual
from Monitoring.SourceIdentification import independent_cascade_opt
from Monitoring.SourceIdentification import imeterOpt
...
def getInfectedOfCascadeByNode(pathname, steps, random_node, out_file):
    ...
    infected_nodes = independent_cascade(graph, {random_node}, steps)
    ...
    return infected_nodes, getInfectedSubgraph(graph, infected_nodes), random_node, getInfectedCalibratedSubgraph(graph,
                                                                                                                  infected_nodes)
...
def getInfectedOfCascade(pathname, steps, out_pathname_graph=None, nodes=None, edges=None):
    ...
    if out_pathname_graph is not None:
        buildGraphOnFile(out_pathname_graph, nodes, edges)

    # print 'Graph:'
    # print list(graph.edges_iter(data='weight', default=1))
    ...
    # pageranks = nx.pagerank(graph, weight='weight')
...
def process(steps, pathname, random_node=None, out_file=None, file_version=False):
    ...
    while True:
        ...
        infected_nodes, infected_subgraph, random_node, infected_calibrated_subgraph = getInfectedOfCascadeByNode(
            pathname, steps, random_node, out_file)
        if infected_nodes is None:
            return
        if len(infected_nodes) > 2550:
            if file_version:
                out_file.write('Too much infected nodes ' + str(len(infected_nodes)) + '\n')
    ...
    branch = maximum_spanning_arborescence_manual(pathname_branch, random_node, attr='weight')
    # print 'branch'
    ...
    branch_calibrated = maximum_spanning_arborescence_manual(pathname_branch_cal, random_node, attr='weight')
    ...
    for node in infected_nodes:
        ...
        if source:
            ...
            if node != random_node:
                if file_version:
                    out_file.write(
                        'Distance from ' + str(random_node) + ' and ' + str(node) + ' is ' + str(dist) + '\n')
                else:
...
if __name__ == "__main__":
    ...
    for node in nodes:
        ...
        process(steps, pathname_graph, random_node=str(node), out_file=open(pathname_output + str(node), 'a'),
                file_version=True)
        n_nodes -= 1
    # def runProcess(node):
