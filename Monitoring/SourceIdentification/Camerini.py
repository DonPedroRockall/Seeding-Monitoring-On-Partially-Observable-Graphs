    ...
    def find_roots_branching(self, k, root='R', scores=False, subgraphs=None):
        if subgraphs is not None:
            ...
            if len(subgraphs) >= k:
                ...
                subgraphs_and_shares = [[subgraph, self.get_graph_score(subgraph),
                                         int(k / len(subgraphs)) if len(subgraph.edges()) != 0 else 1] for subgraph in
                                        subgraphs]
                ...
                for elem in subgraphs_and_shares:
                    ...
                    if elem[2] > 1:
                        ...
                        while len(roots) < elem[2]:
                            ...
                            self._init(elem[0])
                            print('Elem[0] components',
                                  len(list(nx.components.weakly_connected_components(self.graph))))
