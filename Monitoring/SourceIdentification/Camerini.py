...
class MultiDiGraphEdgeKey(nx.MultiDiGraph):
    ...
    def add_edge(self, u_for_edge, v_for_edge, key=None, **attr):
        ...
        if key in self.edge_index:
            ...
            if (u_for_edge != uu) or (v_for_edge != vv):
                raise Exception("Key {0!r} is already in use.".format(key))

        self._cls.add_edge(u_for_edge, v_for_edge, key=key, **attr)
        self.edge_index[key] = (u_for_edge, v_for_edge, self.succ[u_for_edge][v_for_edge][key])
...
class CameriniAlgorithm:
    ...
    def _init(self, graph=None, Y=nx.DiGraph(), Z=nx.DiGraph()):
        ...
        for key, (u, v, data) in enumerate(graph.edges(data=True)):
            if (u, v) not in Z.edges():
                cpy = data.copy()
                self.graph.add_edge(u, v, key, **cpy)
    ...
    def best_incoming_edge(self, node, graph):
        ...
        for u, v, key, data in graph.in_edges([node], data=True, keys=True):
            # print('edge', u, v, data)
            if max_weight <= self.find_weight(data):
                max_weight = self.find_weight(data)
                e = (u, v, key, data)
        return e
    ...
    def best(self, root):
        ...
        for u, v, key, data in M.edges(data=True, keys=True):
            data['origin'] = (u, v, key, {self.attr: self.find_weight(data)})
        ...
        if str(root) in C:  # Modification: if necessary to avoid exception
            del C[str(root)]
        exposed_nodes = [node for node in nodes]
        if root in exposed_nodes:  # Modification: if necessary to avoid exception
        ...
        for node in list(self.graph.nodes()):
            while True:
                ...
                if key not in self.graph.edge_index:
                    break
            self.graph.add_edge(root, node, key, weight=w)
    ...
    def find_roots(self, branching):
        ...
        for node in branching.nodes():
            ...
            if len(in_neigh) == 0 or in_neigh == [("R", node)]:
                roots.append(node)
        return roots
            ...
            if len(subgraphs) >= k:
                ...
                for elem in subgraphs_and_shares:
                    ...
                    if elem[2] > 1:
                        ...
                        root_ = self.find_roots(arborescence)
                        if len(root_) > 1:
                            raise Exception("Looking for one root")
                        root_ = root_[0]
                        score = self.get_graph_score(arborescence)
                        scores[root_] = {'score': score, 'arborescence': arborescence}
    ...
    def find_weight(data):
        if "weight" in data:
            return data["weight"]
        elif "act_prob" in data:
            return data["act_prob"]
        elif "attr_dict" in data:
            if "weight" in data["attr_dict"]:
                return data["attr_dict"]
            elif "act_prob" in data["attr_dict"]:
                return data["act_prob"]
        else:
            raise AttributeError("No weight found in edge")
